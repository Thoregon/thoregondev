...
export default class FsMapper {
    ...
    async process(req) {
        ...
        try {
            ...
            console.log("$$ fsmapper error", e.stack ? e.stack : e.message);
    ...
    async head(req) {
        ...
        let stat     = await fs.stat(fspath);
        let res      = this.buildResStatFS(stat, req.path);
        if (res.type === 'dir') {
            // check if there is an index.mjs or an index.reliant.mjs
            res.hasindex = isFile(path.join(fspath, 'index.mjs')) || isFile(path.join(fspath, 'index.reliant.mjs'));
        }
        return res;
    }
